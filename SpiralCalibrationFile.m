...
%% input parameters
outputFolder = 'D:\1 4D OCT\Scanpattern\patternUpdate_08_26_2020\scannerfeedbackTest\';
...
%% read averaged feedback signal
filename = 'D:\1 4D OCT\Scanpattern\patternUpdate_08_26_2020\scannerfeedbackTest\2000\XYposinterp_2000_19_11_v1.bin';
fid = fopen (filename,'r');

avg_fb = fread(fid, 'single');
fclose(fid);
size_avg_fb = size (avg_fb);
avg_fb = reshape(avg_fb,[(size_avg_fb(1)/2),2]);


avg_fb(:,1) = avg_fb(:,1)./max(avg_fb(:,1));
avg_fb(:,2) = avg_fb(:,2)./max(avg_fb(:,2));

avg_fb =avg_fb(1:end-450,:);

% figure; 
% plot(avg_fb(1:end-450,1),avg_fb(1:end-450,2))


max_aScanPositionsXmm = max(abs(aScanPositionsXmm));
max_aScanPositionsYmm = max(abs(aScanPositionsYmm));

aScanPositionsXmmcalib = avg_fb(:,1)*max_aScanPositionsXmm;
aScanPositionsYmmcalib = avg_fb(:,2)*max_aScanPositionsYmm;
% 
% aScanPositionsXmmcalib (end-450:end) = 0;
% aScanPositionsYmmcalib (end-450:end) = 0;

%% calculate original A-scan positions on rect grid
aScanPositionsXrectGridInd = (aScanPositionsXmmcalib + maxRadiusMM) / fovMM * (rectGridSize-1) +1;
aScanPositionsYrectGridInd = (aScanPositionsYmmcalib + maxRadiusMM) / fovMM * (rectGridSize-1) +1;

% scanTable(1:48970,5) = aScanPositionsXrectGridInd;
% scanTable(1:48970,6) = aScanPositionsYrectGridInd;


scanTable =scanTable(1:end-450,:);
scanTable(:,5) = aScanPositionsXrectGridInd;
scanTable(:,6) = aScanPositionsYrectGridInd;
...
%check remap file
figure;
plot(scanTable(:,7),scanTable(:,8),'.')
hold on
plot(scanTable(:,9),scanTable(:,10),'.')

scanTable(61051:61500,:) =0;
%% plots
figure(1)
...
for ii = 5%:2:size(scanTable,2)
    plot(scanTable(:,ii), scanTable(:,ii+1), '.r')
%     plot(scanTable(:,2), scanTable(:,ii+3), '.y')
end
...
scanTable(60951:end,:)=0;
scanTable = scanTable(:,1:10);


figure;
plot(scanTable(:,5),scanTable(:,6),'.');
...
filename = strcat(outputFolder, num2str(patternnumber),'_Spiral_2000_11_19_v1.bin');
fid = fopen(filename,'w');
...
%% plot resampled data
scanTableRes = scanTable (:,7:10);


for i = 1:49100
    table(i*2-1,1)=scanTableRes(i,1);
    table(i*2-1,2)= scanTableRes(i,2);
    table(i*2,1)=scanTableRes(i,3);
    table(i*2,2)=scanTableRes(i,4);
end
table( all(table ==0, 2), :) = [];